# Lesson 06 - Aktivitas dan Ajakan

## Tujuan
- Memahami partikel objek `を`.
- Memahami kata kerja aktivitas harian (makan, minum, baca, lihat, dengar).
- Mampu membuat ajakan dengan `ませんか` dan `ましょう`.

## Cakupan Materi
- Kosakata aktivitas dan objek.
- Pola grammar:
  - `N を Vます`
  - `N を します`
  - `Vませんか`
  - `Vましょう`

## Hasil Belajar
Setelah lesson ini, kamu diharapkan bisa:
- Menyebut aktivitas dengan objek secara benar.
- Menanyakan kegiatan lawan bicara dengan bentuk formal.
- Mengajak orang melakukan aktivitas sederhana.

## Status Media
- Daftar kebutuhan gambar: `media/images/README.md`
- Daftar kebutuhan audio: `media/audio/README.md`
- Atribusi sumber (jika pakai aset eksternal): `media/source-attribution.md`
- Status saat ini:
  - Gambar `.svg` inti lesson 06 sudah ditambahkan.
  - Audio `.mp3` draft lesson 06 sudah ditambahkan.
